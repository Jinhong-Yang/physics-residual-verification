...
\title{Supplementary Material\\\large Protocol-Constrained Residual Adaptation with Frozen Vision--Language Features for Rigid-Body Inference}
...
C1: 29.95\% complete-model improvement & Main result JSON and family action-loss archive; portable paired-statistic replay & Synthetic new-geometry analytic action utility; most gain is numerical.\\
...
\begin{table}[h]\centering\small
\caption{All six point-error protection cells. Both conventions use the same 72 geometry families.}
\begin{tabular}{llrrl}\toprule Convention & Protocol & Candidate (mm) & Reference (mm) & Reference\\\midrule
...
\end{tabular}\end{table}
All six meet both the 10\% relative and 0.10 mm absolute harm guards. These values preserve the original cell-specific comparator rule; they are not newly estimated cell confidence intervals. Sliding inference is shared by the two conventions, which explains the identical multiple-force and unforced-slide rows.

\section{Public timing, selection, and uncertainty audit}
The fixed selection samples a permutation of episode indices 10--1499 with seed 20260924: the first 64 become development candidates and the next 128 confirmation candidates. The local plan and individual selected files are included; a fresh census of the pinned remote population is not included in this analysis.
...
The nominal 90\% region uses radii 0.165462 m for numerical and 0.162973 m for gated prediction. An isotropic Gaussian scale is obtained by dividing the radius by $\sqrt{\chi^2_{2,0.9}}$. This fitted parametric interpretation is used for NLL; it does not transform the empirical radius into a calibrated Gaussian posterior. Object/parameter/environment overlap across episode splits remains unverified.

\input{support_results}

\input{video_control_results}

\input{group_holdout_results}

\section{Scope of available evidence}
The archives identify computational artifacts rather than distinct independent replications. The original public development result remains below its 10\% criterion. The controls do not isolate language pretraining, validate a general coordinate mask, or establish universal safety or coverage guarantees. Independent calibration and controlled end-to-end latency comparisons are not available. The new material holdout retains the historical architecture-selection and shared-asset limitations described above.
...
\end{verbatim}
The support and video studies additionally require SciPy. Their feature caches and numerical kernels enable CPU refitting and scoring. Input hashes and the analysis plans are supplied under \path{evidence/support/} and \path{evidence/video_control/}. The prior-lineage command checks the supplied identifier/hash tables and selection receipts; it does not rehash the complete original image collection.
